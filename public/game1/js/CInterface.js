function CInterface(iCurBet, iTotBet, iMoney) {
    ...
    this._init = function (iCurBet, iTotBet, iMoney) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosAudio = {
                x: _oButExit.getX() - oSprite.width / 2,
                y: oSprite.height / 2 + 10,
            };
        ...
        _oSpinBut = new CTextButton(
            1090 + oSprite.width / 2,
            CANVAS_HEIGHT - oSprite.height / 2,
            oSprite,
            "",
            FONT_GAME,
            "#ffde00",
    ...
    this.refreshMoney = function (iMoney, bet,val) {
        $.post("update1", { _token: $('meta[name=csrf-token]').attr('content'), amount: iMoney, type: val,bet:bet,user_id:user_id}, function (t) {
            var amount=bet+JSON.parse(t);
            _oMoneyText.refreshText(
                TEXT_MONEY + "\n" + amount + TEXT_CURRENCY
            );
        });
    };
