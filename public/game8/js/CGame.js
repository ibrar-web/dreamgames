function CGame(oData){
    ...
    this.spinWheel = function(){
        ...
        _oInterface.refreshCredit(_iCurCredit,1);
    ...
    this.setNewRound = function(){
        ...
        _oInterface.refreshCredit(_iCurCredit,2);
    ...
    this.releaseWheel = function(){
        ...
        _iCurCredit += 0;
        //console.log('win',_iCurCredit);
        _iBankCash -= WHEEL_SETTINGS[_iCurWin].prize * _iMultiply;
        
        $(s_oMain).trigger("save_score",[_iCurCredit]);
        if(WHEEL_SETTINGS[_iCurWin].prize * _iMultiply>0){
            $.post("update8", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iCurCredit+WHEEL_SETTINGS[_iCurWin].prize * _iMultiply,bet:_iCurBet,user_id:user_id}, function (t) {
                //_oCreditNum.refreshText(TEXT_CURRENCY + t);
            });
        }
		if(WHEEL_SETTINGS[_iCurWin].prize * _iMultiply>0){
            _iCurCredit+=_iCurBet;
            _oInterface.refreshCredit(_iCurCredit+_iCurBet,3);
        }
        _oInterface.animWin();
