function CInterface(oBgContainer) {
    ...
    this.refreshCredit = function (iValue, type, bet) {
        if (type == 2) {
            _oCreditNum.refreshText(sprintf(TEXT_CURRENCY, iValue));
            return;
        }
        $.post("update47", { _token: $('meta[name=csrf-token]').attr('content'), amount: iValue, user_id: user_id, bet: bet }, function (t) {
            _oCreditNum.refreshText(sprintf(TEXT_CURRENCY, t));
        });
    };
