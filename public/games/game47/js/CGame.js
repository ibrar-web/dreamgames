function CGame(oData) {
    ...
    this._placeBet = function () {
        ...
        _oInterface.refreshCredit(_iCurCredit.toFixed(2) / 1, 1, _iCurBet);
    ...
    this.ballArrived = function (iDestCol) {
        ...
        var val = _iCurCredit + PRIZE[iDestCol] * _iMultiply;
        if (PRIZE[iDestCol] * _iMultiply>0) {
            $.post("update47", { _token: $('meta[name=csrf-token]').attr('content'), amount: val, user_id: user_id, bet: _iCurBet }, function (t) {
                _iCurCredit += bet;
            });
        }
        ...
        _oInterface.refreshCredit(_iCurCredit.toFixed(2) / 1, 2, _iCurBet);
        this.checkEndGame();
    ...
    this.restartGame = function () {
        ...
        _oScoreBasketController.refreshText(_iMultiply);
        _oInterface.refreshCredit(_iCurCredit, 3, _iCurBet);
        _oInterface.refreshBet(_iCurBet);
        _oInterface.showControls();
    };
