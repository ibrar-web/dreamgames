function CGame(oData){
    ...
    this._checkDeckLength = function(){
     ...
     this.assignWin = function(iRet){
        ...
        _iMoney = parseFloat(_iMoney.toFixed(2));
        _iGameCash -= _iCurWin;
        var amo=_iMoney+_iCurWin;
        amo = parseFloat(amo.toFixed(2));
        if(_iCurWin>0){
            _oInterface.refreshWin(_iCurWin);
            $.post("update18", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iMoney+_iCurWin,bet:_iCurBet,user_id:user_id }, function(t) {
            
            });  
            _oInterface.refreshMoney(_iMoney+_iCurBet,_iCurBet,3); 
        }
	...
	_bBlock = true;
        ...
        _iCurBet = iNewBet;
        _oInterface.refreshMoney(_iMoney,_iCurBet,9);
        _oPayTable.setCreditColumn(_iCurCreditIndex);
