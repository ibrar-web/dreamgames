function CInterface(iMoney, iBet) {
    ...
    this._init = function(iMoney, iBet) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosFullscreen = { x: _pStartPosExit.x - oSprite.width - 10, y: _pStartPosExit.y };
        }
    ...
    this.refreshMoney = function(iMoney, iBet,type) {
        if(type==3){
            _oMoneyText.refreshText(iMoney);
        }
        $.post("update18", { _token: $('meta[name=csrf-token]').attr('content'), amount: iMoney,user_id:user_id }, function(t) {
