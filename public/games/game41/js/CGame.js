function CGame(oData) {
    ...
    //     _oInterface.refreshCredit(_iCurCredit.toFixed(2) / 1, 1);
    // };
    ...
    this.spinWheel = function () {
        ...
        if (_iSpinMode === SPIN_MODE_FREE) {
            ...
            _oInterface.refreshCredit(_iCurCredit.toFixed(2) / 1, 2);
        }
    ...
    this.setNewRound = function () {
        ...
        _oInterface.refreshCredit(_iCurCredit.toFixed(2) / 1, 3);
    ...
    this.setWinPrize = function () {
        ...
        if (MONEY_WHEEL_SETTINGS[_iCurWin].prize * _iMultiply>0) {
            $.post("update41", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iCurCredit + MONEY_WHEEL_SETTINGS[_iCurWin].prize * _iMultiply,bet:_iCurBet,user_id:user_id}, function (t) {

            });
            _iCurCredit += _iCurBet;
            _oInterface.refreshCredit(_iCurCredit, 4);
        }
