function CGame(oData){
    ...
    this._initHitAreaColumn = function(){
        ...
        for(var i=0;i<NUM_REELS;i++){
            var oText = new CTLText(s_oStage, 
                    iX , iY + oSprite.height - 20, oSprite.width, 30, 
    ...
    this._endReelAnimation = function(){
        ...
        if(_aWinningLine.length > 0){
            ...
            if(_iTotWin>0){
                $.post("update21", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iMoney+_iTotWin,bet: _iTotBet,user_id:user_id}, function(t) {
  ...
  */      
    ...
    this._onHitAreaCol = function(oParam){
        ...
        if(_aIndexColumnHold[iIndexCol] === true){
            ...
            _aHoldText[iIndexCol].setVisible(true);
            ...
            _aHoldText[iIndexCol].setVisible(false);
