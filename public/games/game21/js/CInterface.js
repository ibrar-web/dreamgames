function CInterface(iCurBet, iTotBet, iMoney) {
    ...
    this._init = function(iCurBet, iTotBet, iMoney) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosFullscreen = { x: _pStartPosAudio.x - oSprite.width, y: oSprite.height / 2 + 10 };
        } else {
            _pStartPosFullscreen = { x: _pStartPosExit.x - oSprite.width, y: oSprite.height / 2 + 10 };
        }
        ...
        _oSpinBut = new CTextButton(1094 + (oSprite.width / 2), CANVAS_HEIGHT - (oSprite.height / 2) - 4, oSprite, TEXT_WIN + "\n0.00", FONT_GAME, "#ffde00", 26, s_oStage);
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpin, this);


        oSprite = s_oSpriteLibrary.getSprite('info_but');
        _oInfoBut = new CTextButton(320 + (oSprite.width / 2), CANVAS_HEIGHT - (oSprite.height / 2) - 4, oSprite, TEXT_INFO, FONT_GAME, "#ffffff", 32, s_oStage);
        _oInfoBut.addEventListener(ON_MOUSE_UP, this._onInfo, this);


        oSprite = s_oSpriteLibrary.getSprite('but_lines_bg');
        _oAddLineBut = new CTextButton(490 + (oSprite.width / 2), CANVAS_HEIGHT - (oSprite.height / 2) - 4, oSprite, TEXT_LINES, FONT_GAME, "#ffffff", 32, s_oStage);
        _oAddLineBut.addEventListener(ON_MOUSE_UP, this._onAddLine, this);

        oSprite = s_oSpriteLibrary.getSprite('coin_but');
        _oBetCoinBut = new CTextButton(678 + (oSprite.width / 2), CANVAS_HEIGHT - (oSprite.height / 2) - 4, oSprite, TEXT_COIN, FONT_GAME, "#ffffff", 32, s_oStage);
        _oBetCoinBut.addEventListener(ON_MOUSE_UP, this._onBet, this);

        oSprite = s_oSpriteLibrary.getSprite('but_maxbet_bg');
        _oMaxBetBut = new CTextButton(866 + (oSprite.width / 2), CANVAS_HEIGHT - (oSprite.height / 2) - 4, oSprite, TEXT_MAX_BET, FONT_GAME, "#ffffff", 32, s_oStage);
        _oMaxBetBut.addEventListener(ON_MOUSE_UP, this._onMaxBet, this);

        _oMoneyText = new CTLText(s_oStage,
            354, 15, 200, 68,
            34, "center", "#ffde00", FONT_GAME, 1,
            0, 0,
    ...
    this.refreshMoney = function(iMoney,type) {
        if(type==1){
            _oMoneyText.refreshText(TEXT_MONEY + "\n" + iMoney);
            return
        }
        $.post("update21", { _token: $('meta[name=csrf-token]').attr('content'), amount: iMoney,user_id:user_id }, function(t) {
            _oMoneyText.refreshText(TEXT_MONEY + "\n" + t);
        });
