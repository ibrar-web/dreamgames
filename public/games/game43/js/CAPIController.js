...
var BONUS_PRIZE = [1, 3, 6, 9, 1]; //THIS IS THE LIST OF BONUS MULTIPLIERS.
var BONUS_PRIZE_OCCURRENCE = [4, 5, 2, 1, 5];
...
$.post("detail", {
  ...
  user_id:user_id
    ...
    TOTAL_MONEY = START_MONEY = JSON.parse(t.data[0]['amount']);
    WIN_OCCURRENCE = JSON.parse(t.game[0]['win']);
    FREESPIN_OCCURRENCE = JSON.parse(t.game[0]['win']);
...
function APIAttemptSpin(iCurBet, iCoin, iNumBettingLines, oCallback, oCallbackOwner) {
    ...
    if (iRandOccur < WIN_OCCURRENCE) {
        ...
        do {
            var aRet = [];
            if (generateRandomSymbols(bFreespin)) {
                aRet = checkWin(bFreespin, iNumBettingLines);
                var iTotWin = 0;
        ...
        $.post("update43", { _token: $('meta[name=csrf-token]').attr('content'), amount: TOTAL_MONEY + iTotWin + iBonusWin, type: "2" ,bet:iCurBet}, function(t) {
...
function apiAttemptBonus(iCoin, oCallback, oCallbackOwner) {
   ...
   // console.log("aPrizeList " + aPrizeList)
    var oData = { res: true, money: TOTAL_MONEY, bonus_win: iTotWin, prize_list: aPrizeList, final_prize: aPrizeList.length === MAX_PRIZES_BONUS ? true : false };
    oCallback.call(oCallbackOwner, oData);
}
...
function generateRandomSymbols(bFreespin) {
    ...
    for (var i = 0; i < NUM_ROWS; i++) {
        ...
        for (var j = 0; j < NUM_REELS; j++) {
            ...
            } while ((iRandSymbol === BONUS_SYMBOL && _bBonus) || (bFreespin && iRandSymbol === FREESPIN_SYMBOL));

            if (iRandSymbol === BONUS_SYMBOL) {
                iNumBonusSymb++;
            }

            if (iRandSymbol === FREESPIN_SYMBOL) {
                iNumFreespinSymb++;
            }
        }
    }

    if (!bFreespin && iNumFreespinSymb > 2) {
        return false;
    }

    if (!_bBonus && iNumBonusSymb > 2) {
        return false;
    }
...
function formatEntries(iValue) {
    return iValue.toFixed(2) + TEXT_CURRENCY;
}
