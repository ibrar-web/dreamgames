...
var CANVAS_WIDTH = 1500, CANVAS_HEIGHT = 640, EDGEBOARD_X = 300, EDGEBOARD_Y = 0, FPS_TIME = 1E3 / 24, DISABLE_SOUND_MOBILE = !1, FONT_GAME = "Franklin Gothic Heavy", FONT_PAYTABLE = "Arial", STATE_LOADING = 0, STATE_MENU = 1, STATE_HELP = 1, STATE_GAME = 3, GAME_STATE_IDLE = 0, GAME_STATE_SPINNING = 1, GAME_STATE_SHOW_ALL_WIN = 2, GAME_STATE_SHOW_WIN = 3, GAME_STATE_BONUS = 4, REEL_STATE_START = 0, REEL_STATE_MOVING = 1, REEL_STATE_STOP = 2, ON_MOUSE_DOWN = 0, ON_MOUSE_UP = 1, ON_MOUSE_OVER = 2, ON_MOUSE_OUT = 3, ON_DRAG_START = 4, ON_DRAG_END = 5, REEL_OFFSET_X = 380, REEL_OFFSET_Y =
    ...
    BONUS_FREESPIN = 1,
    userbet = 0, BONUS_WHEEL = 2, WHEEL_SETTINGS, SEGMENT_ROT = 18, TIME_ANIM_IDLE = 1E4, MIN_FAKE_SPIN = 3, WHEEL_SPIN_TIMESPEED = 2600, ANIM_SPIN_TIMESPEED = 50, TIME_ANIM_WIN = 5E3, ANIM_WIN1_TIMESPEED = 300, ANIM_WIN2_TIMESPEED = 50, LED_SPIN = 3, ANIM_IDLE1_TIMESPEED = 2E3, ANIM_IDLE2_TIMESPEED = 100, ANIM_IDLE3_TIMESPEED = 150, STATE_BONUS_IDLE = 0, STATE_BONUS_SPIN = 1, STATE_BONUS_WIN = 2, STATE_BONUS_LOSE = 3, NUM_SPIN_FOR_ADS, ENABLE_FULLSCREEN, ENABLE_CHECK_ORIENTATION, SHOW_CREDITS;
...
function CInterface(a, f, d) {
    ...
    this.refreshButtonPos = function (a, b) {
        t.setPosition(e - a, b + g); !1 !== DISABLE_SOUND_MOBILE && !1 !== s_bMobile || z.setPosition(l - a, b + h); L && screenfull.isEnabled && I.setPosition(m - a, k + b)
    };
    this.refreshMoney = function (a) {
        $.post("update45", { _token: $('meta[name=csrf-token]').attr('content'), amount: a, user_id: user_id }, function (t) {
    ...
    if (d > s_aSession.iMoney) _dieError("INVALID BET: " + d + ",money:" + s_aSession.iMoney);
        ...
        var e = s_aSession.bBonus = 0, g = 0; if (s_aSession.iSlotCash < s_aSession.min_win * f) return generLosingPattern(), 1 === s_aSession.bFreeSpin && (--s_aSession.iTotFreeSpin, 0 > s_aSession.iTotFreeSpin && (s_aSession.iTotFreeSpin = 0, s_aSession.bFreeSpin = 0)), "res=true&win=false&pattern=" + JSON.stringify(_aFinalSymbols) +
            "&money=" + s_aSession.iMoney + "&freespin=" + s_aSession.iTotFreeSpin + "&bonus=false&bonus_prize=-1&bonus_win=0&cash=" + s_aSession.iSlotCash; if (Math.floor(100 * Math.random()) < s_aSession.win_occurrence) {
                ...
                } var k; m = 0; do { generateRandomSymbols(e, g); var l = checkWin(e, g, a), h = 0; for (k = 0; k < l.length; k++)h += l[k].amount; h *= f; var b = 0; k = -1; if (1 === g) { s_aSession.bBonus = 1; k = []; for (b = 0; b < s_aSession.bonus_prize_occur.length; b++)for (var c = s_aSession.bonus_prize_occur[b], t = 0; t < c; t++)k.push(b); k = k[Math.floor(Math.random() * k.length)]; b = s_aSession.bonus_prize[k] * f } m++ } 
                while (0 === l.length || b + h > s_aSession.iSlotCash || b + h < d); 
                s_aSession.iMoney = s_aSession.iMoney + 0+0; 
                $.post("update45", { _token: $('meta[name=csrf-token]').attr('content'), amount: s_aSession.iMoney + h + b, bet: userbet, user_id: user_id }, function (t) {
