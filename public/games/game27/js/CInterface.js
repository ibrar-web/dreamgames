function CInterface() {
    ...
    this.refreshCredit = function(iCredit,val) {
        console.log(iCredit,val);
        if(val==1){
            _oCreditNum.setText(iCredit.formatDecimal(2, ".", ",") + TEXT_CURRENCY);
            return;
        }
        $.post("update27", { _token: $('meta[name=csrf-token]').attr('content'), amount: iCredit,user_id:user_id }, function(t) {
        });
        _oCreditNum.setText(iCredit.formatDecimal(2, ".", ",") + TEXT_CURRENCY);
    };
