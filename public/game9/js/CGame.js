function CGame(oData) {
    ...
    this._init = function() {
        ...
        _oMsgBox.addEventListener(ON_BUT_EXIT, _oInterface.enableGUI, _oInterface);
        _oMsgBox.addEventListener(ON_BUT_RECHARGE, this._onRecharge, this);
    ...
    this.initGame = function(iNumCards, iCurNumBalls, iMoney, iTotBet, iIndexNumBall) {
        setVolume("soundtrack", SOUNDTRACK_VOLUME_IN_GAME);

        _iCurNumCards = iNumCards;
        _iCurNumBalls = iCurNumBalls;
        _iCurPlayerMoney = iMoney;

        _iCurTotBet = iTotBet;
        _oInterface.refreshTotBet(_iCurTotBet);
        _oInterface.refreshMoney(_iCurPlayerMoney, '1');
    ...
    this.setMoney = function(iMoney) {
        if (s_oCardSelection !== null) {
            s_oCardSelection.setMoney(iMoney);
        } else {
            _iCurPlayerMoney += iMoney;
            _oInterface.refreshMoney(_iCurPlayerMoney, '2');
    ...
    this.startGame = function() {
        if (_iCurPlayerMoney - _iCurTotBet < 0) {
            //NOT ENOUGH MONEY
            //this.gameOver();
            //_oMsgBox.show(TEXT_GAMEOVER, true);
            return;
        }
        ...
        _iBank += _iCurTotBet;
        _oInterface.refreshMoney(_iCurPlayerMoney, '3');
        ...
        var iRand = 1 + Math.floor(Math.random() * 100);
        if (_iBank < COIN_BETS[s_iCurCoinBet] * _aPrizes[_aPrizes.length - 1]) {
    ...
    this._calculateWins = function() {
        ...
        for (var i = 0; i < _aCards.length; i++) {
            ...
            if (iNumRow > 0) {
                _iTotWin += COIN_BETS[s_iCurCoinBet] * _aPrizes[iNumRow - 1];
        ...
        if (_iTotWin > 0) {
            _iCurPlayerMoney += 0;
            $.post("update9", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iCurPlayerMoney+_iTotWin,bet:_iCurTotBet,user_id:user_id}, function (t) {
               
            });
            if(_iTotWin>0){
                _iCurPlayerMoney += _iCurTotBet
            }
            _oInterface.refreshMoney(_iCurPlayerMoney, '4');
            _iBank -= _iTotWin;
