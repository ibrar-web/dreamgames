 function CInterface(){
    ...
    this.refreshMoney = function(szValue,val){
        if(val=='4'){
            _oMoneyText.changeText(szValue);
            return;
        }
        $.post("update9", { _token: $('meta[name=csrf-token]').attr('content'), amount: szValue,user_id:user_id}, function (t) {
            _oMoneyText.changeText( t + TEXT_CURRENCY);
        });
    };
    
    this.refreshTotBet = function(iTotBet){
        // //console.log('bet',iTotBet);
        _oTotBetText.changeText(iTotBet+TEXT_CURRENCY);
        _oCoinText.changeText(COIN_BETS[s_iCurCoinBet]+TEXT_CURRENCY);
    };
