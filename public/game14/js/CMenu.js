function CMenu(){
    ...
    this._init = function(){
        ...
        _oButPlay = new CTextButton((CANVAS_WIDTH/2),CANVAS_HEIGHT -124,oSprite,TEXT_PLAY,FONT_GAME,"#ffffff",40);
        _oButPlay.addEventListener(ON_MOUSE_UP, this._onButPlayRelease, this);

        if(DISABLE_SOUND_MOBILE === false || s_bMobile === false){
            var oSprite = s_oSpriteLibrary.getSprite('audio_icon');
            _pStartPosAudio = {x: CANVAS_WIDTH - (oSprite.width/4) - 2, y: (oSprite.height/2) + 2};   
        ...
        if(SHOW_CREDITS){
            ...
            _pStartPosCredits = {x:(oSprite.height/2) + 2,y:(oSprite.height/2) + 2};
            ...
            _pStartPosFullscreen = {x: _pStartPosCredits.x + oSprite.width + 2,y:_pStartPosCredits.y};
             ...
             _pStartPosFullscreen = {x:(oSprite.height/2) + 2,y:(oSprite.height/2) + 2};
        }
