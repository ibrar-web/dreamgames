    ...
    this._checkContinueGame = function(){        
        ...
        for(var i=0; i<PAYOUTS[_iTotalNum-1].hits.length; i++){
            if(PAYOUTS[_iTotalNum-1].hits[i] === _iHitsNumber){
                ...
                //adding game result back to game
                // console.log(iTotalWin);
                _iCurPlayerMoney += 0;
                _iBank -= iTotalWin;
                _oPayoutsTable.showWin(iTotalWin);
                _oPayoutsTable.highlightWin(_iHitsNumber);
                $.post("update5", { _token: $('meta[name=csrf-token]').attr('content'), amount: _iCurPlayerMoney + iTotalWin,type:'2',bet:_iCurBet,user_id:user_id}, function (t) {
                    //console.log('done');
                });
                _iCurPlayerMoney+=_iCurBet;
                break;
