function CInterface(){
    ...
    this.setCurBet = function(iCurBet){
        // console.log('Bet',iCurBet);
        bet=iCurBet;
        _oBetAmountText.refreshText(iCurBet.toFixed(2) + TEXT_CURRENCY);
    };

    this.refreshMoney = function(iStartMoney, iMoney,val){
        // console.log(iMoney,val);
        // console.log(iStartMoney);
        // console.log('bet',bet);
        //start money includes bet if win edit from cseat
        $.post("update4", { _token: $('meta[name=csrf-token]').attr('content'), amount: iStartMoney ,bet:bet,reward:iMoney,user_id:user_id }, function (t) {
